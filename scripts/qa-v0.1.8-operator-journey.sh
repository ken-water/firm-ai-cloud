...
OUTPUT_DIR="${OUTPUT_DIR:-.run/qa/v0.1.8-journey-${RUN_ID}}"
...
fatal() {
  printf '[qa-v0.1.8][ERROR] %s\n' "$*" >&2
  exit 1
}

usage() {
  cat <<'USAGE'
Runbook preset + replay validation (v0.1.8)

Usage:
  bash scripts/qa-v0.1.8-operator-journey.sh [options]
...
stage_policy_read() {
  set_stage_context \
    "/api/v1/ops/cockpit/runbook-templates, /execution-policy, /presets" \
    "Verify runbook template/policy/preset read path and RBAC mapping."
  ...
  request_api GET "/api/v1/ops/cockpit/runbook-templates/presets?template_key=dependency-check&limit=20"
  status_is_success "${LAST_STATUS}" || {
    echo "runbook presets load failed, status=${LAST_STATUS}"
    cat "${LAST_BODY_FILE}" || true
    return 1
  }

  echo "policy/preset read validated mode=${POLICY_MODE}"
  return 0
}
...
stage_policy_write() {
  ...
  request_api PUT "/api/v1/ops/cockpit/runbook-templates/execution-policy" \
    '{"mode":"hybrid_live","live_templates":["dependency-check"],"max_live_step_timeout_seconds":10,"allow_simulate_failure":false,"note":"qa v0.1.8 policy update"}'
...
stage_preset_create() {
  set_stage_context \
    "/api/v1/ops/cockpit/runbook-templates/presets (POST)" \
    "Create dependency-check preset with reviewed params and preflight list."

  if (( READ_ONLY_MODE == 1 )); then
    echo "read-only mode enabled, skipping preset create write-path"
    return 2
  fi

  request_api POST "/api/v1/ops/cockpit/runbook-templates/presets" \
    "{\"template_key\":\"dependency-check\",\"name\":\"qa-${RUN_ID}\",\"description\":\"qa preset\",\"execution_mode\":\"simulate\",\"params\":{\"asset_ref\":\"qa-${RUN_ID}\",\"dependency_target\":\"127.0.0.1:8080\",\"probe_timeout_seconds\":5},\"preflight_confirmations\":[\"confirm_probe_source\",\"confirm_dependency_owner\",\"confirm_ticket_context\"],\"note\":\"qa preset create\"}"

  if [[ "${LAST_STATUS}" == "403" ]]; then
    echo "preset create forbidden under current RBAC; skipped"
    cat "${LAST_BODY_FILE}" || true
    return 2
  fi

  status_is_success "${LAST_STATUS}" || {
    echo "preset create failed, status=${LAST_STATUS}"
    cat "${LAST_BODY_FILE}" || true
    return 1
  }

  PRESET_ID="$(jq -r '.item.id // empty' "${LAST_BODY_FILE}")"
  [[ -n "${PRESET_ID}" ]] || {
    echo "preset create response missing preset id"
    cat "${LAST_BODY_FILE}" || true
    return 1
  }

  echo "preset create validated preset_id=${PRESET_ID}"
  return 0
}
...
stage_simulate_execute() {
  set_stage_context \
    ...
    "Execute dependency-check baseline to produce replay source execution."
  ...
  request_api POST "/api/v1/ops/cockpit/runbook-templates/dependency-check/execute" \
    "{\"execution_mode\":\"simulate\",\"params\":{\"asset_ref\":\"qa-${RUN_ID}\",\"dependency_target\":\"127.0.0.1:8080\",\"probe_timeout_seconds\":5},\"preflight_confirmations\":[\"confirm_probe_source\",\"confirm_dependency_owner\",\"confirm_ticket_context\"],\"evidence\":{\"summary\":\"qa v0.1.8 simulate execution\",\"ticket_ref\":\"QA-SIM-${RUN_ID}\",\"artifact_url\":\"https://example.invalid/qa/${RUN_ID}/simulate\"},\"note\":\"qa simulate\"}"
...
stage_replay_execute() {
  set_stage_context \
    ...
    "Replay baseline execution and verify replay_source_execution_id linkage."
...
stage_records_verify() {
  set_stage_context \
    "/api/v1/ops/cockpit/runbook-templates/presets and /executions/{id}" \
    "Ensure preset list and replay execution detail include source linkage payload."

  request_api GET "/api/v1/ops/cockpit/runbook-templates/presets?template_key=dependency-check&limit=50"
  status_is_success "${LAST_STATUS}" || {
    echo "preset list failed, status=${LAST_STATUS}"
    cat "${LAST_BODY_FILE}" || true
    return 1
  }
  ...
  if [[ -n "${PRESET_ID}" ]]; then
    local preset_found
    preset_found="$(jq -r --arg pid "${PRESET_ID}" '.items[]? | select((.id|tostring)==$pid) | .id' "${LAST_BODY_FILE}" | head -n 1)"
    [[ -n "${preset_found}" ]] || {
      echo "preset ${PRESET_ID} not found in list"
      cat "${LAST_BODY_FILE}" || true
      return 1
    }
  fi
  ...
  if [[ -n "${REPLAY_EXECUTION_ID}" ]]; then
    ...
    [[ "${replay_source}" == "${SIM_EXECUTION_ID}" ]] || {
      echo "detail replay_source_execution_id mismatch: ${replay_source}"
      cat "${LAST_BODY_FILE}" || true
      return 1
    }
    [[ "${runtime_replay_source}" == "${SIM_EXECUTION_ID}" ]] || {
      echo "runtime_summary replay source mismatch: ${runtime_replay_source}"
      cat "${LAST_BODY_FILE}" || true
      return 1
    }
  fi

  echo "preset/replay records validated"
  return 0
}

stage_preset_delete() {
  set_stage_context \
    "/api/v1/ops/cockpit/runbook-templates/presets/{id} (DELETE)" \
    "Delete QA preset and keep preset library clean."

  if (( READ_ONLY_MODE == 1 )); then
    echo "read-only mode enabled, skipping preset delete write-path"
    return 2
  fi

  [[ -n "${PRESET_ID}" ]] || {
    echo "preset id missing, skip delete"
    return 2
  }

  request_api DELETE "/api/v1/ops/cockpit/runbook-templates/presets/${PRESET_ID}"

  if [[ "${LAST_STATUS}" == "403" ]]; then
    echo "preset delete forbidden under current RBAC; skipped"
    cat "${LAST_BODY_FILE}" || true
    return 2
  fi

  status_is_success "${LAST_STATUS}" || {
    echo "preset delete failed, status=${LAST_STATUS}"
    cat "${LAST_BODY_FILE}" || true
    return 1
  }

  echo "preset delete validated preset_id=${PRESET_ID}"
  return 0
}
...
write_summary() {
  ...
  {
    echo "# v0.1.8 Operator Journey Validation Summary"
    echo
    echo "- Run ID: ${RUN_ID}"
...
main() {
  ...
  run_stage "policy_read" "runbook policy/preset read path" stage_policy_read
  run_stage "policy_write" "runbook policy write path" stage_policy_write
  run_stage "preset_create" "runbook preset create path" stage_preset_create
  ...
  run_stage "records_verify" "runbook preset/replay records path" stage_records_verify
  run_stage "preset_delete" "runbook preset delete path" stage_preset_delete

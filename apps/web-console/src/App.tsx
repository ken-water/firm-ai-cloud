...
type RunbookTemplateReplayResponse = {
  generated_at: string;
  template: RunbookTemplateCatalogItem;
...
type RunbookAnalyticsPolicyItem = {
  policy_key: string;
...
type RunbookAnalyticsPolicyResponse = {
  generated_at: string;
  policy: RunbookAnalyticsPolicyItem;
};

type RunbookAnalyticsPolicyDraft = {
  failure_rate_threshold_percent: string;
...
type RunbookRiskOwnerDirectoryItem = {
  owner_key: string;
  display_name: string;
  owner_type: "team" | "user" | "group" | "external";
  owner_ref: string;
...
type RunbookRiskOwnerDirectoryResponse = {
  generated_at: string;
  total: number;
  items: RunbookRiskOwnerDirectoryItem[];
};

type RunbookRiskOwnerRoutingRuleItem = {
  rule_id: number;
  template_key: string;
  execution_mode: "simulate" | "live" | null;
...
type RunbookRiskOwnerRoutingRulesResponse = {
  generated_at: string;
  total: number;
  items: RunbookRiskOwnerRoutingRuleItem[];
};

type RunbookRiskOwnerReadinessItem = {
  template_key: string;
  template_name: string;
  owner_key: string | null;
  owner_label: string | null;
  ...
  readiness_status:
    ...
    | "missing_notification_template"
    | "missing_owner_directory"
    | "owner_disabled"
    | "missing_notification_target"
    | "missing_notification_channel"
    | "missing_notification_subscription"
    | "ready";
  gap_reason: string;
  notification_template_enabled: boolean;
  matched_channel_count: number;
  matched_subscription_count: number;
};

type RunbookRiskOwnerReadinessResponse = {
  generated_at: string;
  total: number;
...
type OperatorIntegrationBootstrapCatalogItem = {
  integration_key: "operator_notifications" | "ticket_followup_policy";
  name: string;
  category: string;
  summary: string;
  status: "ready" | "action_required";
  recommended_apply_order: number;
  auto_applicable: boolean;
  gap_reason: string;
  required_inputs: string[];
  default_payload: {
    channel_name?: string | null;
    channel_type?: "email" | "webhook" | null;
    target?: string | null;
...
type OperatorIntegrationBootstrapCatalogResponse = {
  generated_at: string;
  total: number;
  recommended_next_key: string | null;
  items: OperatorIntegrationBootstrapCatalogItem[];
};

type ApplyOperatorIntegrationBootstrapResponse = {
  applied: boolean;
...
type RunbookExecutionPresetItem = {
  ...
  execution_mode: "simulate" | "live";
  params: Record<string, unknown>;
  preflight_confirmations: string[];
  updated_by: string;
  created_at: string;
  updated_at: string;
};

type RunbookExecutionPresetListResponse = {
  generated_at: string;
  total: number;
  limit: number;
  offset: number;
  items: RunbookExecutionPresetItem[];
};

type RunbookExecutionPresetDetailResponse = {
  generated_at: string;
  item: RunbookExecutionPresetItem;
};

type RunbookExecutionPresetDraft = {
  name: string;
  description: string;
};
...
const defaultRunbookExecutionPresetDraft: RunbookExecutionPresetDraft = {
  name: "",
  description: ""
};
...
const defaultRunbookAnalyticsPolicyDraft: RunbookAnalyticsPolicyDraft = {
...
function buildRunbookAnalyticsPolicyDraft(policy: RunbookAnalyticsPolicyItem): RunbookAnalyticsPolicyDraft {
  return {
    failure_rate_threshold_percent: String(policy.failure_rate_threshold_percent),
    minimum_sample_size: String(policy.minimum_sample_size),
    note: policy.note ?? ""
  };
}
...
function buildIntegrationBootstrapDrafts(payload: OperatorIntegrationBootstrapCatalogResponse | null) {
  const drafts: Record<string, Record<string, string>> = {};
  for (const item of payload?.items ?? []) {
    drafts[item.integration_key] = {
      channel_name: item.default_payload.channel_name ?? "",
      channel_type: item.default_payload.channel_type ?? "email",
      target: item.default_payload.target ?? "",
      escalation_owner: item.default_payload.escalation_owner ?? ""
    };
  }
  return drafts;
}
...
export function App() {
  ...
  const [runbookPresets, setRunbookPresets] = useState<RunbookExecutionPresetItem[]>([]);
  ...
  const [runbookAnalyticsPolicy, setRunbookAnalyticsPolicy] = useState<RunbookAnalyticsPolicyItem | null>(null);
  const [runbookAnalyticsPolicyDraft, setRunbookAnalyticsPolicyDraft] =
    useState<RunbookAnalyticsPolicyDraft>(defaultRunbookAnalyticsPolicyDraft);
  ...
  const [runbookRiskAlerts, setRunbookRiskAlerts] = useState<RunbookRiskAlertResponse | null>(null);
  ...
  const [runbookPresetDraft, setRunbookPresetDraft] = useState<RunbookExecutionPresetDraft>(
    defaultRunbookExecutionPresetDraft
  );
  ...
  const [loadingRunbookAnalyticsPolicy, setLoadingRunbookAnalyticsPolicy] = useState(false);
  ...
  const [savingRunbookAnalyticsPolicy, setSavingRunbookAnalyticsPolicy] = useState(false);
  const [savingRunbookRiskOwnerDirectory, setSavingRunbookRiskOwnerDirectory] = useState(false);
  const [savingRunbookRiskOwnerRoutingRules, setSavingRunbookRiskOwnerRoutingRules] = useState(false);
  const [replayingRunbookExecutionId, setReplayingRunbookExecutionId] = useState<number | null>(null);
  ...
  const loadRunbookAnalyticsPolicy = useCallback(async () => {
    setLoadingRunbookAnalyticsPolicy(true);
    setError(null);
    try {
      const response = await apiFetch(`${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/analytics/policy`);
      if (!response.ok) {
        throw new Error(await readErrorMessage(response));
      }
      const payload: RunbookAnalyticsPolicyResponse = await response.json();
      setRunbookAnalyticsPolicy(payload.policy);
      setRunbookAnalyticsPolicyDraft(buildRunbookAnalyticsPolicyDraft(payload.policy));
      return payload.policy;
    } catch (err) {
      setError(err instanceof Error ? err.message : "unknown error");
      setRunbookAnalyticsPolicy(null);
      setRunbookAnalyticsPolicyDraft(defaultRunbookAnalyticsPolicyDraft);
      return null;
    } finally {
      setLoadingRunbookAnalyticsPolicy(false);
    }
  }, []);

  const loadRunbookRiskAlerts = useCallback(async () => {
    setLoadingRunbookRiskAlerts(true);
    setError(null);
    try {
      const params = new URLSearchParams();
      ...
      const response = await apiFetch(
        `${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/analytics/alerts?${params.toString()}`
      );
      if (!response.ok) {
        throw new Error(await readErrorMessage(response));
      }
      const payload: RunbookRiskAlertResponse = await response.json();
      setRunbookRiskAlerts(payload);
      setRunbookAnalyticsPolicy(payload.policy);
      setRunbookAnalyticsPolicyDraft(buildRunbookAnalyticsPolicyDraft(payload.policy));
      return payload;
    } catch (err) {
      setError(err instanceof Error ? err.message : "unknown error");
      setRunbookRiskAlerts(null);
      return null;
    } finally {
      setLoadingRunbookRiskAlerts(false);
    }
  }, [
  ...
  const loadRunbookRiskOwnerDirectory = useCallback(async () => {
    setLoadingRunbookRiskOwnerDirectory(true);
    setError(null);
    try {
      const response = await apiFetch(`${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/analytics/owners`);
      if (!response.ok) {
        throw new Error(await readErrorMessage(response));
      }
      const payload: RunbookRiskOwnerDirectoryResponse = await response.json();
      ...
      setLoadingRunbookRiskOwnerDirectory(false);
    }
  }, []);

  const loadRunbookRiskOwnerRoutingRules = useCallback(async () => {
    setLoadingRunbookRiskOwnerRoutingRules(true);
    setError(null);
    try {
      const response = await apiFetch(`${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/analytics/owner-routing-rules`);
      if (!response.ok) {
        throw new Error(await readErrorMessage(response));
      }
      ...
      setLoadingRunbookRiskOwnerRoutingRules(false);
    }
  }, []);

  const loadRunbookRiskOwnerReadiness = useCallback(async () => {
    setLoadingRunbookRiskOwnerReadiness(true);
    setError(null);
    try {
      const response = await apiFetch(`${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/analytics/owner-readiness`);
      if (!response.ok) {
        throw new Error(await readErrorMessage(response));
      }
      const payload: RunbookRiskOwnerReadinessResponse = await response.json();
      setRunbookRiskOwnerReadiness(payload);
      return payload;
    } catch (err) {
      setError(err instanceof Error ? err.message : "unknown error");
      setRunbookRiskOwnerReadiness(null);
      return null;
    } finally {
      setLoadingRunbookRiskOwnerReadiness(false);
    }
  }, []);
  ...
  const loadIntegrationBootstrapCatalog = useCallback(async () => {
    setLoadingIntegrationBootstrapCatalog(true);
    setError(null);
    try {
      const response = await apiFetch(`${API_BASE_URL}/api/v1/ops/cockpit/integrations/bootstrap`);
      if (!response.ok) {
        throw new Error(await readErrorMessage(response));
      }
      const payload: OperatorIntegrationBootstrapCatalogResponse = await response.json();
      setIntegrationBootstrapCatalog(payload);
      setIntegrationBootstrapDrafts((prev) => ({
        ...buildIntegrationBootstrapDrafts(payload),
        ...prev
      }));
      return payload;
    } catch (err) {
      setError(err instanceof Error ? err.message : "unknown error");
      setIntegrationBootstrapCatalog(null);
      return null;
    } finally {
      setLoadingIntegrationBootstrapCatalog(false);
    }
  }, []);
  ...
  const applyIntegrationBootstrap = useCallback(async (integrationKey: string) => {
    ...
    const draft = integrationBootstrapDrafts[integrationKey] ?? {};
    setRunningIntegrationBootstrapKey(integrationKey);
    setRunbookNotice(null);
    setError(null);
    try {
      const response = await apiFetch(`${API_BASE_URL}/api/v1/ops/cockpit/integrations/bootstrap/apply`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({
          ...
          channel_name: trimToNull(draft.channel_name ?? ""),
          channel_type: trimToNull(draft.channel_type ?? ""),
          target: trimToNull(draft.target ?? ""),
          escalation_owner: trimToNull(draft.escalation_owner ?? "")
        })
      });
      if (!response.ok) {
        throw new Error(await readErrorMessage(response));
      }
      const payload: ApplyOperatorIntegrationBootstrapResponse = await response.json();
      const resultSummary = payload.results.map((item) => `${item.resource_kind}:${item.operation}`).join(", ");
      setRunbookNotice(
        payload.results.length > 0
          ? `Integration bootstrap applied for ${payload.integration_key}: ${resultSummary}.`
      ...
      setRunningIntegrationBootstrapKey(null);
    }
  }, [
    canWriteCmdb,
  ...
  const createRunbookRiskAlertTicket = useCallback(async (templateKey: string) => {
    ...
    try {
      ...
      const routedOwner =
        payload.ticket_link.owner_route?.owner_label
        ?? payload.ticket_link.owner_route?.owner
        ?? payload.ticket_link.ticket_assignee
        ?? "-";
  ...
  const saveRunbookAnalyticsPolicy = useCallback(async () => {
    ...
    const failureRateThresholdPercent = Number.parseInt(
      runbookAnalyticsPolicyDraft.failure_rate_threshold_percent.trim(),
      10
    );
    if (!Number.isFinite(failureRateThresholdPercent) || failureRateThresholdPercent < 1 || failureRateThresholdPercent > 100) {
      setError("Failure rate threshold must be an integer between 1 and 100.");
      return null;
    }

    const minimumSampleSize = Number.parseInt(runbookAnalyticsPolicyDraft.minimum_sample_size.trim(), 10);
    if (!Number.isFinite(minimumSampleSize) || minimumSampleSize < 1 || minimumSampleSize > 500) {
      setError("Minimum sample size must be an integer between 1 and 500.");
      return null;
    }

    setSavingRunbookAnalyticsPolicy(true);
    setRunbookNotice(null);
    setError(null);
    try {
      const response = await apiFetch(`${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/analytics/policy`, {
        ...
        body: JSON.stringify({
          failure_rate_threshold_percent: failureRateThresholdPercent,
          minimum_sample_size: minimumSampleSize,
          note: trimToNull(runbookAnalyticsPolicyDraft.note)
        })
      });
      if (!response.ok) {
        throw new Error(await readErrorMessage(response));
      ...
      setRunbookAnalyticsPolicyDraft(buildRunbookAnalyticsPolicyDraft(payload.policy));
      setRunbookNotice(
        `Runbook analytics risk policy updated: failure_rate_threshold_percent=${payload.policy.failure_rate_threshold_percent}, minimum_sample_size=${payload.policy.minimum_sample_size}.`
      );
      await loadRunbookRiskAlerts();
      return payload.policy;
    } catch (err) {
      ...
      setSavingRunbookAnalyticsPolicy(false);
    }
  }, [canWriteCmdb, loadRunbookRiskAlerts, runbookAnalyticsPolicyDraft, t]);

  const saveRunbookRiskOwnerDirectory = useCallback(async () => {
    if (!canWriteCmdb) {
      setError(t("auth.messages.forbiddenAction"));
      return null;
    }

    const items = runbookRiskOwnerDirectory.map((item) => ({
      owner_key: item.owner_key.trim(),
      display_name: item.display_name.trim(),
      owner_type: item.owner_type,
      owner_ref: item.owner_ref.trim(),
      notification_target: trimToNull(item.notification_target ?? ""),
      note: trimToNull(item.note ?? ""),
      is_enabled: item.is_enabled
    }));
    if (items.some((item) => item.owner_key.length === 0 || item.display_name.length === 0 || item.owner_ref.length === 0)) {
      setError("Owner key, display name, and owner ref are required.");
      return null;
    }

    setSavingRunbookRiskOwnerDirectory(true);
    setRunbookNotice(null);
    setError(null);
    try {
      const response = await apiFetch(`${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/analytics/owners`, {
      ...
      setSavingRunbookRiskOwnerDirectory(false);
    }
  }, [
    canWriteCmdb,
    loadGoLiveReadiness,
  ...
  const saveRunbookRiskOwnerRoutingRules = useCallback(async () => {
    if (!canWriteCmdb) {
      setError(t("auth.messages.forbiddenAction"));
      return null;
    }

    const items = runbookRiskOwnerRoutingRules.map((item) => ({
      template_key: item.template_key.trim(),
    ...
    if (items.some((item) => item.template_key.length === 0 || item.owner_key.length === 0)) {
      setError("Template key and owner key are required for routing rules.");
      return null;
    }

    setSavingRunbookRiskOwnerRoutingRules(true);
    setRunbookNotice(null);
    setError(null);
    try {
      const response = await apiFetch(`${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/analytics/owner-routing-rules`, {
      ...
      setSavingRunbookRiskOwnerRoutingRules(false);
    }
  }, [
    canWriteCmdb,
    loadGoLiveReadiness,
  ...
  const loadRunbookExecutionPresets = useCallback(async () => {
    setLoadingRunbookPresets(true);
    setError(null);
    try {
      const params = new URLSearchParams({
        limit: "80",
      ...
      const response = await apiFetch(`${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/presets?${params.toString()}`);
      if (!response.ok) {
        throw new Error(await readErrorMessage(response));
      }
      const payload: RunbookExecutionPresetListResponse = await response.json();
      const items = payload.items ?? [];
      setRunbookPresets(items);
      setSelectedRunbookPresetId((prev) => {
        if (prev.length > 0 && items.some((item) => String(item.id) === prev)) {
          return prev;
        }
      ...
      setLoadingRunbookPresets(false);
    }
  }, [selectedRunbookTemplateKey]);
  ...
  const applyRunbookExecutionPreset = useCallback((preset: RunbookExecutionPresetItem | null) => {
    ...
    const template = runbookTemplates.find((item) => item.key === preset.template_key) ?? null;
    if (!template) {
      setError(`Preset template '${preset.template_key}' is not available in current catalog.`);
      return;
    }

    setSelectedRunbookTemplateKey(preset.template_key);
    setRunbookExecutionMode(preset.execution_mode);
    setRunbookParamDraft(() => {
    ...
    setRunbookPreflightDraft(() => {
      ...
      const confirmed = new Set(preset.preflight_confirmations ?? []);
      for (const item of template.preflight ?? []) {
        next[item.key] = confirmed.has(item.key);
      }
      return next;
    });
    setSelectedRunbookPresetId(String(preset.id));
    setRunbookNotice(
      `Preset '${preset.name}' applied for template '${template.name}' in mode '${preset.execution_mode}'.`
    );
  }, [runbookTemplates]);

  const createRunbookExecutionPreset = useCallback(async () => {
    if (!canWriteCmdb) {
      setError(t("auth.messages.forbiddenAction"));
      return null;
    }
    ...
    const presetName = runbookPresetDraft.name.trim();
    if (presetName.length === 0) {
      setError("Runbook preset name is required.");
      return null;
    }
    ...
    setSavingRunbookPreset(true);
    setError(null);
    setRunbookNotice(null);
    try {
      const response = await apiFetch(`${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/presets`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({
          ...
          name: presetName,
          description: trimToNull(runbookPresetDraft.description),
      ...
      const payload: RunbookExecutionPresetDetailResponse = await response.json();
      await loadRunbookExecutionPresets();
      setSelectedRunbookPresetId(String(payload.item.id));
      setRunbookPresetDraft(defaultRunbookExecutionPresetDraft);
      setRunbookNotice(
        `Runbook preset '${payload.item.name}' saved for template '${payload.item.template_name}'.`
      );
      return payload.item;
    } catch (err) {
      setError(err instanceof Error ? err.message : "unknown error");
    ...
    runbookPresetDraft.description,
    runbookPresetDraft.name,
    runbookTemplates,
    selectedRunbookTemplateKey,
    t
  ]);

  const deleteRunbookExecutionPreset = useCallback(async () => {
    if (!canWriteCmdb) {
      setError(t("auth.messages.forbiddenAction"));
      return null;
    }
    const presetId = Number.parseInt(selectedRunbookPresetId, 10);
    if (!Number.isFinite(presetId) || presetId <= 0) {
      setError("Runbook preset selection is required.");
      return null;
    }
    ...
    try {
      const response = await apiFetch(
        `${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/presets/${presetId}`,
        { method: "DELETE" }
      );
      if (!response.ok) {
        throw new Error(await readErrorMessage(response));
      ...
      await loadRunbookExecutionPresets();
      setSelectedRunbookPresetId("");
      setRunbookNotice(`Runbook preset #${presetId} deleted.`);
      return true;
    } catch (err) {
      setError(err instanceof Error ? err.message : "unknown error");
      return null;
  ...
  }, [canWriteCmdb, loadRunbookExecutionPresets, selectedRunbookPresetId, t]);

  const replayRunbookTemplateExecution = useCallback(async (executionId: number) => {
    if (!canWriteCmdb) {
      setError(t("auth.messages.forbiddenAction"));
      return null;
    }
    if (!Number.isFinite(executionId) || executionId <= 0) {
      setError("Runbook execution id is required for replay.");
      return null;
    }

    setReplayingRunbookExecutionId(executionId);
    setError(null);
    setRunbookNotice(null);
    try {
      const evidenceSummary = runbookEvidenceDraft.summary.trim();
      const payload: Record<string, unknown> = {};
      const overrideNote = trimToNull(runbookEvidenceDraft.note);
      if (overrideNote) {
        payload.note = overrideNote;
      }
      ...
      const response = await apiFetch(
        `${API_BASE_URL}/api/v1/ops/cockpit/runbook-templates/executions/${executionId}/replay`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
      ...
      setReplayingRunbookExecutionId(null);
    }
  }, [
    canWriteCmdb,
    loadRunbookAnalyticsSummary,
  ...
  useEffect(() => {
    void loadRunbookAnalyticsPolicy();
  ...
  useEffect(() => {
    void loadRunbookRiskAlerts();
  }, [loadRunbookRiskAlerts]);

  useEffect(() => {
    void loadRunbookRiskOwnerDirectory();
  }, [loadRunbookRiskOwnerDirectory]);

  useEffect(() => {
    void loadRunbookRiskOwnerRoutingRules();
  }, [loadRunbookRiskOwnerRoutingRules]);
